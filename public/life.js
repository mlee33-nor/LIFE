...
function bars(sessions,field) {
  ...
  return sorted.map(([key,minutes])=>`<div class="time-bar"><div><strong>${esc(field==='activity'?(names[key]||key):key)}</strong><span>${duration(minutes)} · ${total?Math.round(minutes/total*100):0}%</span></div><div class="time-track"><span style="width:${total?minutes/total*100:0}%;background:${colors[key]||'#9166e5'}"></span></div></div>`).join('')||'<p class="time-empty">No matching sessions logged.</p>';
}
